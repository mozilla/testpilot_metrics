 ...
 * Status: Draft
 * URL: https://sql.telemetry.mozilla.org/queries/1028
...
SELECT
  num.week                                                                   AS week,
  CONCAT(CONCAT(num.type,': '),CONCAT(CONCAT(num.label,' / '), denom.label)) AS label,
  100.0*num.cnt/(num.cnt + denom.cnt)                                        AS pct
FROM tmp_weekly AS num
  JOIN tmp_weekly AS denom ON(
      num.week=denom.week
      AND num.label_id=denom.label_id-1
      AND num.type=denom.type
    )
